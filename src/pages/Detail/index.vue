            ...
            <div class="chooseArea">
              ...
              <dl v-for="(item, index) in spuSaleAttrList" :key="index">
                ...
                <dd
                  ...
                  :key="attrValue.id"
                  @click="isChecked(item.spuSaleAttrValueList,index)"
                >{{attrValue.saleAttrValueName}}</dd>
              </dl>
            </div>
            <div class="cartWrap">
              <div class="controls">
                <input autocomplete="off" class="itxt" v-model="defaultNum" @change="$event.target.value*1 > 1? defaultNum = $event.target.value*1 : defaultNum = 1"/>
                <a href="javascript:" class="plus" @click="defaultNum++">+</a>
                <a href="javascript:" class="mins" @click="defaultNum<=1?1:defaultNum--">-</a>
              </div>
              <div class="add">
                <a href="javascript:" @click="getAddCartOrUpdate">加入购物车</a>
              </div>
            </div>
          </div>
        </div>
...
export default {
  ...
  methods: {
    ...
    isChecked(atrValueList, index) {
      // 排它
      atrValueList.forEach(item => {
        item.isChecked = "0";
      });
      atrValueList[index].isChecked = "1";
    },
    async getAddCartOrUpdate(){
      try {
      const result = await this.$store.dispatch('getAddCartOrUpdate',{skuId:this.skuInfo.id,skuNum:this.defaultNum})
      sessionStorage.setItem('INFO',JSON.stringify(this.skuInfo))
      this.$router.push(`/shopcart/${this.defaultNum}`)
